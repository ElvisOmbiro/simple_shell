...
/**
 * source_history - gets the history file
...
{
        ssize_t fd;
        char *filename = source_history(info);
        list_t *node = NULL;
        ...
        _wrifds(BUF_FLUSH, fd);
        close(fd);
        return (1);
}

/**
 * document_history - reads history from file
 * @info: the parameter struct
 *
...
int document_history(info_t *info)
{
        ...
        struct stat st;
        char *buf = NULL, *filename = source_history(info);
        ...
        for (i = 0; i < fsize; i++)
                ...
                {
                        buf[i] = 0;
                        adds_entrylist(info, buf + last, linecount++);
                        last = i + 1;
                }
        if (last != i)
                adds_entrylist(info, buf + last, linecount++);
...
/**
 * adds_entrylist - adds entry to a history linked list
...
int adds_entrylist(info_t *info, char *buf, int linecount)
{
        ...
        if (info->history)
                node = info->history;
        endlist_add(&node, buf, linecount);
